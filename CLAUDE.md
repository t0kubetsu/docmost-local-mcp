...
- [src/server.rs](src/server.rs) + [src/server/tools.rs](src/server/tools.rs) + [src/server/tools_write.rs](src/server/tools_write.rs) — `DocmostMcpServer { client, tool_router }`, `#[tool_handler] impl ServerHandler`. **21 tools**: 10 read-only (`list_spaces`, `search_docs`, `search_pages` [alias], `get_space`, `get_page`, `list_pages`, `list_child_pages`, `get_comments`, `list_workspace_members`, `get_current_user`, all `read_only_hint = true`) plus 11 **write** tools (`read_only_hint = false`): `create_page`/`update_page` (Markdown → ProseMirror) and `edit_page` (in-place edits on the stored JSON, see [docs/edit-page.md](docs/edit-page.md)) in `tools_page_write.rs`, and `duplicate_page`, `copy_page_to_space`, `move_page`, `move_page_to_space`, `create_space`, `update_space`, `create_comment`, `update_comment` in `tools_write.rs`. The write tools use a separate `#[tool_router(router = write_tool_router)]` merged into the main router in `new()` (`tool_router() + write_tool_router()`), keeping each tools file within the size limit.
